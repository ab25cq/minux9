...
void alloc_prog(char* elf_buf, int elf_buf_size, int fork_flag, int exec_flag, int* child_proc_index, int debug_) {
    ...
    result->process_pages = NULL;
    result->num_process_pages = 0;
    result->process_pages_list = NULL;
    neoc_proc_process_pages_clear(result);

    struct process_pages* first_page = kalloc();
    first_page->process_kalloc_address = NULL;
    first_page->num_process_kalloc_address = 0;
    first_page->process_kalloc_address_list = NULL;
    neoc_process_page_kalloc_clear(first_page);
    neoc_proc_process_pages_add(result, first_page);
    ...
    result->uid = 0;
    result->gid = 0;
    result->supp_gid_list = NULL;
    neoc_proc_supp_gid_clear(result);
...
//printf("ELF_PROG_LOAD i%d\n", i);
        ...
        for (va = seg_start; va < seg_end; va += PGSIZE) {
            ...
            void *pa = kalloc();
            struct process_pages* page = neoc_proc_process_pages_last(result);

            if (!pa) panic("kalloc");
            memset(pa, 0, PGSIZE);

            neoc_process_page_kalloc_add(page, (char*)pa);
            if (page->num_process_kalloc_address >= NUM_PROC_VA_MAX) {
                struct process_pages* new_page = kalloc();
                new_page->process_kalloc_address = NULL;
                new_page->num_process_kalloc_address = 0;
                new_page->process_kalloc_address_list = NULL;
                neoc_process_page_kalloc_clear(new_page);
                neoc_proc_process_pages_add(result, new_page);
            }
...
//printf("vaddr %p exec %d\n", ph->vaddr, exec_flag);
    ...
    if(fork_flag) {
        ...
        neoc_proc_copy_supp_gids(result, parent);
...
void free_proc(struct proc *p) {
    ...
    for (int i=0; i<p->num_process_pages; i++) {
        struct process_pages* page = neoc_proc_process_pages_get(p, i);
        
        for(int j=0; j<page->num_process_kalloc_address; j++) {
            kfree(neoc_process_page_kalloc_get(page, j));
        }
        
        neoc_process_page_kalloc_clear(page);
        kfree(page);
    }
    neoc_proc_process_pages_clear(p);
...
int uvm_alloc(struct proc *p, pagetable_t pagetable, uint64_t old_sz, uint64_t new_sz) {
    ...
    for(; va < new_sz; va += PGSIZE) {
        ...
        struct process_pages* page = neoc_proc_process_pages_last(p);
        
        neoc_process_page_kalloc_add(page, mem);
        ...
        if(page->num_process_kalloc_address >= NUM_PROC_VA_MAX) {
            ...
            new_page->process_kalloc_address_list = NULL;
            neoc_process_page_kalloc_clear(new_page);
            neoc_proc_process_pages_add(p, new_page);
        }
