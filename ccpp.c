...
static void preprocess(FILE *in, FILE *out, const PPOpts *opts, const char *curdir, const char *this_path, MacroTable *ptbl) {
    ...
    if (!ptbl) {
        ...
        const char *sys = "__LINUX__";
        const char *mach = "riscv64";
        int is64 = (sizeof(void*) == 8);
        union { unsigned int u; unsigned char b[4]; } endu; endu.u = 0x01020304u;
        int is_little = (endu.b[0] == 0x04);
        // Common byte/bit and byte order macros
        mtable_set_obj(tbl, "__CHAR_BIT__", "8");
        ...
        // Size-related macros used by many arch headers (approximate typical LP64/ILP32)
        const char *psz = is64 ? "8" : "4";
        mtable_set_obj(tbl, "__SIZEOF_POINTER__", psz);
        mtable_set_obj(tbl, "__SIZEOF_SIZE_T__", psz);
        mtable_set_obj(tbl, "__SIZEOF_SHORT__", "2");
        mtable_set_obj(tbl, "__SIZEOF_INT__", "4");
        mtable_set_obj(tbl, "__SIZEOF_LONG_LONG__", "8");
        // long is pointer-sized on most Unixy LP64/ILP32 targets we care about here
        mtable_set_obj(tbl, "__SIZEOF_LONG__", is64 ? "8" : "4");
        ...
        if (sys && strstr(sys, "Linux")) {
            ...
            // Provide common glibc feature-test defaults to reduce header guard errors
            mtable_set_obj(tbl, "_GNU_SOURCE", "1");
            mtable_set_obj(tbl, "_DEFAULT_SOURCE", "1");
        } else if (sys && strstr(sys, "Darwin")) {
            mtable_set_obj(tbl, "__APPLE__", "1");
            mtable_set_obj(tbl, "__MACH__", "1");
            if (is64) mtable_set_obj(tbl, "__LP64__", "1");
            // Encourage Darwin/POSIX paths in headers
            mtable_set_obj(tbl, "_DARWIN_C_SOURCE", "1");
            mtable_set_obj(tbl, "_POSIX_C_SOURCE", "200809L");
            // Be liberal about feature level to expose modern prototypes
            mtable_set_obj(tbl, "__DARWIN_C_LEVEL", "200809L");
            // Common Darwin convenience macros that appear in public headers
            // Keep them numeric to avoid later identifier lookups in consumers.
            mtable_set_obj(tbl, "__DARWIN_NBBY", "8");
            // On macOS, fd_set uses 32-bit slots; define NFDBITS accordingly.
            mtable_set_obj(tbl, "__DARWIN_NFDBITS", "32");
            // Provide a conservative NULL to satisfy simple uses in headers
            mtable_set_obj(tbl, "__DARWIN_NULL", "0");
            // Some Darwin headers (runetype/ctype) expect _CACHED_RUNES to be
            // a constant; ensure it is defined to a typical value (256) so
            // downstream tools that don't resolve enumerators still work.
            mtable_set_obj(tbl, "_CACHED_RUNES", "256");
